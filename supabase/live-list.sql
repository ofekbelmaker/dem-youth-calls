...
--
--  submit_signup() כבר משבצת נרשם חדש לתור בעצמה (auto-sync.sql).
--  הקובץ הזה מכסה את מה שהיא לא רואה: היא רצה רק במסלול הטופס.
--
--  מה נשאר מחוץ לה:
--    • רכז שמחזיר מישהו לפעילות — status חוזר ל-'active', והוא
--      אמור לחזור לרשימה. אף אחד לא קורא ל-submit_signup בשבילו.
--    • שורה שנוספה ל-people ישירות מהטבלה או מייבוא.
--    • הכיוון ההפוך, שאין לו בכלל מימוש אחר: מי שסומן 'עזב' או
--      'אל תפנו אליו' צריך לרדת מהרשימה, ולא רק להפסיק להיכנס אליה.
--
--  כאן ההצטרפות לרשימה הופכת לתוצאה של המסד עצמו: כל מי שנעשה
--  בר-פנייה נכנס לכל פעולה פתוחה, וכל מי שחדל להיות — יוצא.
--  החפיפה עם submit_signup לא מזיקה: שני הצדדים on conflict do
--  nothing, ושיבוץ קיים לא נכפל.
--
